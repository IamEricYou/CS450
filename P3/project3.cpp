...
const char *WINDOWTITLE = { "Jaehyung You - Project #3" };
...
//백그라운드
const GLfloat BACKCOLOR[ ] = { 0.0745 , 0.2352 , 0.498 , 1. };
...
Display( )
{
	...
	//Image is from  https://www.fileformat.info/format/bmp/sample/index.htm
	unsigned char* Texture1 = BmpToTexture((char*)"temp.bmp", &width, &height);
	glBindTexture(GL_TEXTURE_2D, tex1);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	glColor3f(0.5,0,1); // Color for the sphere
	MjbSphere(1,50,50);
	if(TextureMenu)
	{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "CS450 - Project #3" );
...
DoTextureMenu( int id )
{
	...
	//NoTexture = TextureMenu;
	glutSetWindow( MainWindow );
	glutPostRedisplay( );  
}

void DoImageMenu( int id )
{
	if(id == 0)
	{
		image1 = !id;
...
InitMenus( )
{
    ...
    int texture_menu = glutCreateMenu( DoTextureMenu );
	glutAddMenuEntry( "With no texture", 0 );
	glutAddMenuEntry( "With texture w/ Distortion Off",   1);
	glutAddMenuEntry( "With texture w/ Distortion On",   2);

	int image_menu = glutCreateMenu( DoImageMenu );
	glutAddMenuEntry( "World Map", 0 );
	glutAddMenuEntry( "Mountain", 1 );
	...
	glutAddSubMenu(   "Select Image",       image_menu);
